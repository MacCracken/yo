...
| Field | Value |
|---|---|
| Current version | **0.5.9** — test-gate soundness + `sys_exit_group` portability (0.5.8: toolchain 6.5.35 + taar 0.5.0 dep bump; 0.5.7: AGNOS kernel-leased DNS; 0.5.5: IPv4 codec folded onto taar) |
...
| Cyrius pin | 6.5.35 |
| Tests | 365 assertions in `tests/yo.tcyr` — IPv4-embedded parsing, scope-id parsing (`ipv6_parse_ex`), v4-mapped output formatter, `platform_resolve_ifindex` against `/sys/class/net/lo`. **The gate is sound as of 0.5.9**: all three entry points clamp a non-zero return to 1, so the 256/512/768-failure wait-status truncation that would have scored PASS is closed |
